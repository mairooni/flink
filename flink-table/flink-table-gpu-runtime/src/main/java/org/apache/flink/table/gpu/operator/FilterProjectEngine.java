 ...
 * is handled by running the kernel over the whole buffer and having the caller drain only the
 * first {@code count} positions. Reallocating per batch would put an allocation in the hot path
 * and defeat {@code FIRST_EXECUTION} transfer modes later.
 *
 * <p><b>Transfer modes are deliberately EVERY_EXECUTION here.</b> This is a single-kernel task
 * graph, so there is no intermediate to keep resident and nothing to gain from
 * {@code persistOnDevice}. Residency arrives in P2, when a reduction consumes this kernel's output
 * without a host round trip; the copy-out measured here is exactly what P2 is meant to remove.
 */
public final class FilterProjectEngine implements AutoCloseable {
    ...
    /**
     * @param intensity extra arithmetic rounds per element; 0 selects the plain 2-flop kernel.
     *                  Used to test whether device work can become a meaningful share at all.
     */
    public FilterProjectEngine(
            int batchSize, double mul, double add, double threshold, boolean profile,
            int intensity) {
    ...
    public void open() {
        ...
        TaskGraph graph = new TaskGraph("calc")
                .transferToDevice(DataTransferMode.EVERY_EXECUTION, val);
        if (intensity == 0) {
            graph = graph.task("filterProject", FilterProjectKernels::scaleAndSelect,
                    val, out, mask, mul, add, threshold);
        } else {
            graph = graph.task("filterProject", FilterProjectKernels::heavyScaleAndSelect,
                    val, out, mask, mul, add, threshold, intensity);
        }
    ...
    public void recordBatch(int count, long gatherNanos, Execution execution,
                            int emitted, long drainNanos) {
        metrics.recordBatch(count, emitted, gatherNanos, execution.wallNanos(), drainNanos,
